 ...
 * sorting (TInput = number[]) and searching/graphs (each with their
 * own richer input shapes) combine into one heterogeneous list —
 * this registry only ever stores and looks up definitions, never
 * calls `generateRandomInput` or `run` itself, so it never actually
 * needs to know each entry's specific input shape. The page that
 * does call them re-narrows to the type it knows it's dealing with
 * at that point (see AlgorithmDetailPage).
 */
